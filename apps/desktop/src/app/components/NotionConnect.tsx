...
import { notionAuthClient } from '@/notionAuth';
...
interface Props {
  projectId: string;
  /** owner만 연결·변경할 수 있다. member에게는 연결 상태만 보여준다. */
  canConnect: boolean;
  /** 연결이 완료되어 색인 대상이 바뀌었다. */
  onConnected?: () => void;
}

/** 프로젝트의 노션 워크스페이스 연결 버튼 + 연결 상태 표시. */
export function NotionConnect({ projectId, canConnect, onConnected }: Props) {
  ...
  const refresh = useCallback(async () => {
    try {
      const status = await notionAuthClient.getStatus(projectId);
      setState(
        status.connected
   ...
   */
  const connect = async (previousWorkspaceName?: string) => {
    if (polling.current) return;
    cancelled.current = false;
    setState({ kind: 'connecting', workspaceName: previousWorkspaceName });

    try {
      await openExternal(await notionAuthClient.startConnect(projectId));
  ...
  const cancel = async (previousWorkspaceName?: string) => {
    ...
    try {
      await notionAuthClient.cancel(projectId);
    } catch (err) {
  ...
  if (state.kind === 'connected') {
    return (
      <span className="notion-connect connected">
        ...
        {canConnect && <button onClick={() => connect(state.workspaceName)}>연결 변경</button>}
      </span>
    );
  }
  ...
  if (!canConnect) {
    return <span className="notion-connect">노션 미연결 — 소유자의 연결이 필요합니다</span>;
  }
