...
> ⚠️ `/oauth/notion/session`은 **인증이 필요 없는 공개 엔드포인트**입니다. 서버를 공개 인터넷에 노출하지 말고 사내망이나 VPN 안에서만 열어 주세요.
...
| 증상 | 원인 | 해결 |
| --- | --- | --- |
| 서버가 기동하자마자 죽음 | `GEMINI_API_KEY` 미설정 | `.env`에 키를 넣습니다. 에러 메시지에 발급 URL이 포함되어 있습니다. |
...
> ⚠️ **노션 액세스 토큰이 DB에 평문으로 저장됩니다.** 프로젝트 설계 문서(`.claude/skills/rag-implementation/storage.md`)는 AES-256-GCM 암호화를 요구하지만 아직 구현되지 않았습니다. DB 파일의 접근 권한을 엄격히 관리하세요.

> ⚠️ **여러 팀의 회의록이 하나의 SQLite 파일에 함께 저장됩니다.** 팀 간 격리는 모든 쿼리에 붙는 `connection_id` 조건만으로 이뤄집니다. 설계 문서가 요구하는 연결별 DB 파일 분리는 아직 미구현입니다.

> ⚠️ 서버를 공개 인터넷에 노출하지 마세요. `/oauth/notion/session`이 무인증 엔드포인트이므로 누구나 앱 토큰 발급을 시도할 수 있습니다.
